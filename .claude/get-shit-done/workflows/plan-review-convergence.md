...
```bash
INIT=$(node "D:/SourceCode/Laravel_Test/Test_laravel13_1/.claude/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "D:/SourceCode/Laravel_Test/Test_laravel13_1/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "D:/SourceCode/Laravel_Test/Test_laravel13_1/.claude/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
